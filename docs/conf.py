...
#
# Convert Python API example notebooks to html for PythonAPI/Examples page
#

import glob
exampleDir  = os.path.expandvars("$CONDA_PREFIX/lib/python3.9/site-packages/vapor/example_notebooks")
destDir     = pwd + "/pythonAPIReference/examples"
examples    = glob.glob(exampleDir + "/*.ipynb")

# create examples table of contents to be populated in the 'for' loop below
examplesTOC = open(destDir + ".rst", "w")
examplesTOC.write(
"""Examples
________
...
for notebook in examples:
    # copy raw .html file to /examples directory
    subprocess.call("jupyter nbconvert --execute --to html " + notebook, shell=True)
    html=notebook.replace("ipynb","html")
    subprocess.call("mv " + html + " " + destDir, shell=True)

    # create .rst file that references raw .html file
    rst=html.replace(exampleDir, destDir)
    rst=rst.replace("html","rst")
    file = open(rst, "w")
    title = os.path.basename(rst)
    file.write(title + "\n") 
    file.write(len(title) * "_" + "\n\n") 
    file.write(".. raw:: html\n")
    file.write("   :file: " + os.path.basename(html) + "\n") 
    file.close()

    # add .rst to examples toctree
    examplesTOC.write("   examples/" + os.path.basename(rst) + "\n")
